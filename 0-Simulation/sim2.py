...
)  #permet de gerer les arguments
args = parser.parse_args()
sim = simulation.Simulation("rrr/robot.urdf", fixed=True, panels=True) #crée objet simulation

sliders = {}
target = None
joints = sim.getJoints() #" donne angle moteur"
bx = 0.07
bz = 0.25
...
lastLine = time.time()
lastInverse = 0
targets = {"motor" + str(k + 1): 0 for k in range(3)} #pareil que motor 1 = 0, motor 2 =0 etc
    ...
    if sim.t > 1.0:
        if args.mode == "direct":
            ...
            T = kinematics.computeDK(
                -targets["motor1"], -targets["motor2"], targets["motor3"] #donner x y z bout de la pointe 
            )
